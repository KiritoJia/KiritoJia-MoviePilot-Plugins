...
class AutoSubtitle(_PluginBase):
    ...
    async def on_transfer_complete(self, event) -> None:  # noqa: C901
        ...
        # 字幕落盘逻辑使用本地文件系统，不能直接对 115、CloudDrive2 等远程储存路径
        # 调用 Path.exists()/iterdir()。远程储存应由本地挂载目录或自定义目录扫描处理。
        fileitem = transferinfo.fileitem
        target_storage = str(
            getattr(video_file, "storage", None)
            or getattr(fileitem, "storage", None)
            or ""
        ).strip()
        local_storage_names = {"local", "localdisk", "local storage", "本地", "本地存储", "本地储存"}
        if target_storage and target_storage.lower() not in local_storage_names:
            logger.info(
                f"[AutoSubtitle] 目标文件属于远程储存“{target_storage}”，"
                f"无法直接访问本地路径，跳过字幕下载: {video_file.path}"
            )
            return
        if not await AsyncPath(str(video_file.path)).is_file():
            logger.info(
                f"[AutoSubtitle] 目标文件不在当前容器本地文件系统，跳过字幕下载: {video_file.path}"
            )
            return
        ...
        source_storage = str(getattr(fileitem, "storage", None) or "").strip()
        source_dir = None
        if fileitem and fileitem.path:
            if not source_storage or source_storage.lower() in local_storage_names:
                source_dir = Path(fileitem.path).parent
            else:
                logger.debug(
                    f"[AutoSubtitle] 整理源属于远程储存“{source_storage}”，"
                    f"不使用远程路径作为字幕源目录: {fileitem.path}"
                )
