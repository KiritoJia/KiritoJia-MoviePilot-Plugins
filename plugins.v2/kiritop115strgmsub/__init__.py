...
class KiritoP115StrgmSub(_PluginBase):
    """115网盘订阅追更插件"""

    # 默认地址为空；用户填写的任意 PanSou 地址都应持久化并生效。

    # 插件名称
    plugin_name = "Kirito 115网盘订阅追更"
    # 插件描述
    ...
    # 插件配置项ID前缀
    plugin_config_prefix = "kiritop115strgmsub_"
    plugin_order = 20
    auth_level = 1
    ...
    def init_plugin(self, config: dict = None):
        ...
        if config:
            ...
            # 不再把任何地址当作“默认地址”清除，保留用户保存的自定义地址。
            self._pansou_url = str(config.get("pansou_url") or "").strip().rstrip("/")
