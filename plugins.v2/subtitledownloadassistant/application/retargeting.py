...
from ..domain.enums import FileLocation, MediaType, RecordStatus
...
from .path_mapping import PathMappingResolution, resolve_path
...
class RetargetTarget(Protocol):
    """可供改配使用的 MoviePilot 整理目标。"""
...
class RetargetTargetQueryPort(Protocol):
    ...
    async def get_target(self, history_id: int) -> RetargetTarget | None:
        """返回有效的单文件本地整理历史目标。"""

    async def list_all_targets(self) -> Sequence[RetargetTarget]:
        """返回全部去重后的有效整理历史目标。"""
...
class RetargetResult:
    ...
    def success(self) -> bool:
        ...
        return (self.record is not None or self.preview is not None) and self.error_code is None
...
class BatchRetargetPreviewItem:
    ...
    target_history_id: int | None = None
    target: RetargetTarget | None = None
    preview: RetargetPreview | None = None
    error_code: str | None = None
    message: str | None = None
...
class RetargetService:
    ...
    async def _record_source_path(self, record: MatchRecord) -> Path:
        ...
        if record.location is FileLocation.PLUGIN_DATA:
            path = await self._filesystem.plugin_file_path(record.path)
        else:
    ...
    def _resolve_target(self, target: RetargetTarget) -> PathMappingResolution:
        """使用调用时的当前配置解析整理历史目标路径。"""

        history_path = getattr(target, "history_target_path", None) or target.context.target_path
        resolution = resolve_path(history_path, self._config_provider().path_mappings)
        if resolution.mapping is not None or resolution.resolved_path == target.context.target_path:
            return resolution
        return PathMappingResolution(
            original_path=resolution.original_path,
            resolved_path=target.context.target_path,
        )
    ...
    async def _build_preview(
        ...
        return RetargetPreview(
            ...
            final_subtitle_path=str(destination),
    ...
    @staticmethod
    def _matches_exact_target(record: MatchRecord, target: RetargetTarget) -> bool:
    ...
    def _suggest_target(
        cls,
        record: MatchRecord,
        targets: Sequence[RetargetTarget],
    ) -> RetargetTarget | None:
        """仅在精确匹配结果唯一时返回自动建议目标。"""
    ...
    async def _preview_mapping(
        ...
        all_targets: Sequence[RetargetTarget],
        target_cache: dict[int, RetargetTarget | None],
    ...
    def _mapping_snapshot(resolution: PathMappingResolution) -> PathMappingSnapshot | None:
        ...
        if resolution.mapping is None:
            return None
        return PathMappingSnapshot(**resolution.mapping.as_dict())
    ...
    async def _updated_record(
        ...
        updated.final_subtitle_path = str(destination)
        updated.retarget_history.append(
            RetargetHistoryEntry(
                ...
                old_subtitle_path=str(old_subtitle),
                new_subtitle_path=str(destination),
            )
        )
        return updated
    ...
    async def _retarget_unlocked(self, record_id: str, target_history_id: int) -> RetargetResult:
        ...
        return RetargetResult(record=updated)
    ...
    async def retarget(self, record_id: str, target_history_id: int) -> RetargetResult:
        ...
        async with self._lock, self._mutation_lock:
            return await self._retarget_unlocked(record_id, target_history_id)
