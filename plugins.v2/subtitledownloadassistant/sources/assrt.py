...
SEARCH_CACHE_REGION = "subtitledownloadassistant_source_assrt"
...
class AssrtSource:
    ...
    async def _request_json(
        ...
        try:
            if response is None:
                ...
                raise SourceRequestError(f"ASSRT 请求失败（{'、'.join(attempts)}均无响应）")
            ...
            if response.status_code in {401, 403}:
                raise SourceRequestError(f"ASSRT Token 无效或无权限（HTTP {response.status_code}）")
    ...
    async def download(self, handle: CandidateHandle, directory: Path) -> DownloadedAsset:
        ...
        path = await download_file(AsyncRequestUtils(**_proxy_kwargs(self._active_proxies)), url, directory, file_name)
    ...
    async def refresh(self, manual: bool = False) -> SourceStatus:
        ...
        try:
            ...
            status.health = SourceHealth.ERROR
            status.last_error_at = utc_now()
            status.last_error_summary = f"ASSRT 网络或响应异常（{type(exc).__name__}）"
        status.details = dict(self._last_details)
