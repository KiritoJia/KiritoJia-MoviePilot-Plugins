...
def _proxy_kwargs(proxies: dict[str, str] | None) -> dict[str, Any]:
    ...
    return {"proxies": proxies or {}}


async def request_res_with_proxy_fallback(
    method: str,
    url: str,
    *,
    ...
    **kwargs: Any,
) -> tuple[Any | None, dict[str, Any], dict[str, str]]:
    """优先使用宿主代理，连接无响应时回退直连并返回安全观测。"""

    if method not in {"get_res", "post_res"}:
        raise ValueError(f"不支持的 HTTP 请求方法：{method}")
    candidates: list[tuple[str, dict[str, str]]] = []
    if proxies:
        candidates.append(("代理", proxies))
    candidates.append(("直连", {}))
    attempts: list[str] = []
    for transport, selected_proxies in candidates:
        attempts.append(transport)
        request_options: dict[str, Any] = {"proxies": selected_proxies}
        if headers is not None:
            request_options["headers"] = headers
        request = AsyncRequestUtils(**request_options)
        try:
            response = await getattr(request, method)(url, **kwargs)
        except Exception:  # noqa: BLE001 - 宿主不同版本可能抛出或吞掉连接异常
            response = None
        if response is not None:
            return (
                ...
                {
                    "transport": transport,
                    "proxy_fallback": bool(proxies and transport == "直连"),
                },
                selected_proxies,
            )
    return (
