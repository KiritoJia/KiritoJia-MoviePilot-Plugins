...
class OpenSubtitlesSource:
    ...
    USER_AGENT = "MoviePilot SubtitleDownloadAssistant/1.1.11"
    ...
    def _filter_automatic(
        ...
        for handle in handles:
            ...
            if not candidate_has_supported_chinese(candidate):
    ...
    async def _request_page(self, params: dict[str, Any], page: int) -> dict[str, Any]:
        ...
        self._last_details.update(transport_details)
        self._active_proxies = active_proxies
        try:
            if response is None:
                attempts = transport_details.get("transport_attempts") or ["直连"]
                raise SourceRequestError(f"OpenSubtitles 搜索请求失败（{'、'.join(attempts)}均无响应）")
    ...
    async def _login(self, force: bool = False) -> str:
        ...
        async with self._login_lock:
            ...
            try:
                if response is None:
                    attempts = transport_details.get("transport_attempts") or ["直连"]
                    raise SourceRequestError(f"OpenSubtitles 登录请求失败（{'、'.join(attempts)}均无响应）")
                if response.status_code >= 400:
                    if response.status_code == 429:
                    ...
                    if response.status_code == 403:
                        raise SourceRequestError("OpenSubtitles API Key 无权限（HTTP 403）")
    ...
    async def _download_link(self, file_id: int, retry_auth: bool = True) -> tuple[str, str]:
        ...
        try:
            if response is None:
                attempts = transport_details.get("transport_attempts") or ["直连"]
                raise SourceRequestError(f"OpenSubtitles 下载授权请求失败（{'、'.join(attempts)}均无响应）")
    ...
    async def download(self, handle: CandidateHandle, directory: Path) -> DownloadedAsset:
        ...
        path = await download_file(AsyncRequestUtils(**_proxy_kwargs(self._active_proxies)), link, directory, file_name)
    ...
    async def refresh(self, manual: bool = False) -> SourceStatus:
        ...
        except Exception as exc:  # noqa: BLE001 - 外部字幕源异常必须收敛为安全状态
            status.health = SourceHealth.ERROR
            status.last_error_at = utc_now()
            status.last_error_summary = f"OpenSubtitles 网络或响应异常（{type(exc).__name__}）"
            status.details = {**self._last_details, "session_active": False}
