...
SEARCH_CACHE_REGION = "subtitledownloadassistant_source_moviepilot"
...
class MoviePilotSource:
    ...
    async def _subtitle_site_indexers(self) -> list[dict[str, Any]]:
        ...
        if isinstance(enabled_sites, str):
            try:
                enabled_sites = json.loads(enabled_sites)
            except json.JSONDecodeError:
                enabled_sites = [item.strip() for item in enabled_sites.split(",") if item.strip()]
        if isinstance(enabled_sites, dict):
            enabled_sites = list(enabled_sites.values())
        elif not isinstance(enabled_sites, (list, tuple, set)):
            enabled_sites = []
        ...
        for item in enabled_sites:
            ...
            if value in (None, ""):
                continue
            enabled_site_ids.add(str(value).strip())
        result: list[dict[str, Any]] = []
        for indexer in await SitesHelper().async_get_indexers():
            if not indexer.get("subtitles"):
                continue
            # DSM/MP 不同版本可能分别返回 int、str 或带 id 的对象；统一成
            # 字符串后再比较，避免保存站点配置后全部被误判为未启用。
            if not enabled_site_ids or str(indexer.get("id", "")).strip() in enabled_site_ids:
