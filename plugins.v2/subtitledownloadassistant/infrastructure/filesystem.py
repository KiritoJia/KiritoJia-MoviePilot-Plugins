...
class SubtitleFileSystem:
    ...
    async def stage_file_deletion(self, path: Path) -> Path | None:
        ...
        if not await target.is_file():
            raise IsADirectoryError(str(path))
        for _ in range(8):
            backup = target.with_name(f".{target.name}.subtitledownloadassistant-delete-{uuid.uuid4().hex}")
