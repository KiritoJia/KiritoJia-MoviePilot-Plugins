...
export interface ConfigModel {
  ...
  custom_media_directories: string[]
...
export type NonSensitiveConfig = Pick<
  ...
  | 'format_priority'
  | 'path_mappings'
  | 'custom_media_directories'
  | 'directory_monitor_enabled'
...
export interface CustomDirectoryScanResponse {
  success: boolean
  message: string
...
export interface SearchPlanItem {
  kind: 'id' | 'title' | 'filename' | 'fingerprint' | 'fallback'
  label: string
  query: string | null
  editable: boolean
}
