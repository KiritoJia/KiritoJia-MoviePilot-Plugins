...
<template>
  <VApp :class="shellClass">
    <VAppBar density="compact" flat border>
      <VAppBarTitle class="dev-title">字幕下载助手开发验收</VAppBarTitle>
      <template #append>
    ...
    <VMain>
      ...
      <AppPage v-if="view === 'workbench'" :key="`app-${renderKey}`" :api="api" plugin-id="SubtitleDownloadAssistant" nav-key="main" />
