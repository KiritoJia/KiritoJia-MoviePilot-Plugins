import {
  ...
  type ConfigModel,
  type CustomDirectoryScanResponse,
  type PluginApi,
...
function seedState(): MockState {
  ...
  const records: RecordListItem[] = [
    ...
    },
    {
      ...
      media_title: '示例剧集', year: 2024, media_type: 'tv', season: 1, episode: 4, status: 'staged', source: 'assrt', package_scope: 'season_pack', location: 'plugin_data', path: '/config/plugins/subtitledownloadassistant/staged/record-staged.srt', current_file_path: '/config/plugins/subtitledownloadassistant/staged/record-staged.srt', target_history_id: null, history_target_path: null, target_path: null, created_at: iso(180_000), updated_at: iso(170_000), consumed_at: null,
    },
    {
      id: 'record-unmatched', subtitle_file_name: 'unknown.zh-cn.srt', format: 'SRT', size: 38_400,
      media_title: null, year: null, media_type: 'unknown', season: null, episode: null, status: 'unmatched', source: 'opensubtitles', package_scope: 'unknown', location: 'plugin_data', path: '/config/plugins/subtitledownloadassistant/unmatched/unknown.zh-cn.srt', current_file_path: '/config/plugins/subtitledownloadassistant/unmatched/unknown.zh-cn.srt', target_history_id: null, history_target_path: null, target_path: null, created_at: iso(680_000), updated_at: iso(670_000), consumed_at: null,
    },
  ]
  ...
  const api: PluginApi = {
    ...
    async post<T = unknown>(path: string, payload?: unknown): Promise<T> {
      ...
      if (path === 'storage/list') {
        const requested = String((payload as { path?: string } | undefined)?.path || '/')
        const directoryTree: Record<string, string[]> = {
          '/': ['app', 'config', 'media', 'mnt'],
          '/media': ['电影', '电视剧', 'STRM'],
          '/media/电影': ['华语电影', '外语电影'],
          '/media/电视剧': ['国产剧', '海外剧'],
          '/media/STRM': ['电影', '电视剧'],
        ...
        const current = requested.length > 1 ? requested.replace(/\/+$/, '') : '/'
        return (directoryTree[current] || []).map(name => ({
          name,
    ...
    async delete<T = unknown>(path: string, options?: Record<string, unknown>): Promise<T> {
      ...
      if (path.endsWith('/tasks/terminal')) {
        const terminalIds = new Set(state.tasks.filter(item => isTerminalTask(item.status)).map(item => item.id))
        state.tasks = state.tasks.filter(item => !terminalIds.has(item.id))
        for (const id of terminalIds) delete state.taskDetails[id]
        return {
          success: true,
          message: terminalIds.size ? `已清理 ${terminalIds.size} 条已结束任务` : '没有需要清理的已结束任务',
          deleted_count: terminalIds.size,
          active_count: state.tasks.length,
        } as T
      }
