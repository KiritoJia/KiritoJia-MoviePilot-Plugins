...
const titleId = `subtitle-download-detail-${useId()}`
...
function restoreFocus(): void {
  ...
  const fallback = matchingTrigger
    ...
    || document.getElementById('subtitle-download-workbench-title')
  fallback?.focus({ preventScroll: true })
  clearStoredFocus()
}

function focusKey(element: HTMLElement): string | null {
  return element.getAttribute('data-subtitle-download-detail-trigger')
}

function focusTriggers(key: string | null): HTMLElement[] {
  const group = key?.split(':', 1)[0]
  return Array.from(document.querySelectorAll<HTMLElement>('[data-subtitle-download-detail-trigger]'))
...
<template>
  <VDialog
    ...
    scrollable
    retain-focus
    content-class="subtitle-download-detail-overlay"
...
<style>
.subtitle-download-detail-overlay {
