...
from .enums import PackageScope, SubtitleSource, TranslationType
from .language import candidate_chinese_priority
from .models import MatchRecord, SubtitleCandidate
...
_PACKAGE_ORDER = {
    PackageScope.EPISODE: 0,
    PackageScope.UNKNOWN: 1,
    PackageScope.SEASON_PACK: 2,
}
...
def _source_quality(candidate: SubtitleCandidate) -> tuple[Any, ...]:
    ...
    if candidate.source in {SubtitleSource.SHOOTER, SubtitleSource.THUNDER}:
        return (
            0 if candidate.trusted else 1,
            -(candidate.score or 0),
...
def candidate_rank(
    ...
    *,
    ...
    return (
        candidate_chinese_priority(candidate),
