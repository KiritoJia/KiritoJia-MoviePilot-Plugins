...
interface PluginApi { get<T = unknown>(path: string, options?: Record<string, unknown>): Promise<T> }
  ...
  <section class="emby-config">
    ...
    <form id="missing-form" class="workspace" @submit.prevent="save">
      ...
      <section class="panel history-panel">
        <div class="history-head"><div class="section-head section-head--inline"><div class="section-index">03</div><div><h2>订阅历史</h2><p>展示最近实际创建或复用的订阅，包含对应媒体海报和缺集。</p></div></div><VBtn icon="mdi-refresh" variant="text" aria-label="刷新订阅历史" :loading="scanning" @click="loadSummary" /></div>
