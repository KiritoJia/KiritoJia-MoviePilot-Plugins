...
def _setup_mock_env() -> None:
    ...
    _make_pkg("app.plugins.kiritop115strmhelper")
    _make_pkg("app.plugins.kiritop115strmhelper.core")
    _make_pkg("app.plugins.kiritop115strmhelper.schemas")
    _make_pkg("app.plugins.kiritop115strmhelper.db_manager")
    _make_pkg("app.plugins.kiritop115strmhelper.helper")
    _make_pkg("app.plugins.kiritop115strmhelper.helper.transfer")
    _make_module("app.plugins.kiritop115strmhelper.core.config", configer=SimpleNamespace())
    _make_module(
        "app.plugins.kiritop115strmhelper.schemas.transfer",
        TransferTask=object,
        RelatedFile=FakeRelatedFile,
    )
    _make_pkg("app.plugins.kiritop115strmhelper.helper.strm")
    _make_module("app.plugins.kiritop115strmhelper.core.scrape", media_scrape_metadata=object)
    _make_module("app.plugins.kiritop115strmhelper.db_manager.oper", FileDbHelper=MagicMock)
    _make_module(
        "app.plugins.kiritop115strmhelper.helper.mediainfo_download",
        MediaInfoDownloader=object,
    )
    _make_module(
        "app.plugins.kiritop115strmhelper.helper.mediaserver",
        MediaServerRefresh=object,
        emby_mediainfo_queue=object,
    )
    _make_module(
        "app.plugins.kiritop115strmhelper.utils.path",
        PathUtils=FakePathUtils,
        PathRemoveUtils=object,
    )
    _make_module(
        "app.plugins.kiritop115strmhelper.utils.sentry",
        sentry_manager=SimpleNamespace(
            sentry_hub=SimpleNamespace(capture_exception=lambda *a, **k: None)
        ),
    )
    _make_module(
        "app.plugins.kiritop115strmhelper.utils.strm",
...
def _load_module(rel_path: str) -> Any:
    ...
    :return Any: 已加载模块
    """
    module_name = f"app.plugins.kiritop115strmhelper.{rel_path.replace('/', '.')}"
