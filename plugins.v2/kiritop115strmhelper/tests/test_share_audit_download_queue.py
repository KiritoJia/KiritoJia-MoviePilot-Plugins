...
def _load_queue_module():
    ...
    for package_name in (
        "app",
        "app.chain",
        "kiritop115strmhelper",
        "kiritop115strmhelper.core",
        "kiritop115strmhelper.helper",
        "kiritop115strmhelper.helper.strm",
        "kiritop115strmhelper.helper.strm.share",
        "kiritop115strmhelper.utils",
    ):
    ...
    config_module = ModuleType("kiritop115strmhelper.core.config")
    ...
    install("kiritop115strmhelper.core.config", config_module)
    ...
    i18n_module = ModuleType("kiritop115strmhelper.core.i18n")
    i18n_module.i18n = SimpleNamespace(
        translate=lambda key, **kwargs: translations[key].format(**kwargs)
    )
    install("kiritop115strmhelper.core.i18n", i18n_module)

    message_module = ModuleType("kiritop115strmhelper.core.message")
    message_module.post_message = MagicMock()
    install("kiritop115strmhelper.core.message", message_module)

    cache_module = ModuleType("kiritop115strmhelper.core.cache")
    cache_module.rename_media_fields_cacher = _Cache()
    install("kiritop115strmhelper.core.cache", cache_module)

    rename_module = ModuleType("kiritop115strmhelper.utils.rename_dict")
    ...
    install("kiritop115strmhelper.utils.rename_dict", rename_module)

    limiter_module = ModuleType("kiritop115strmhelper.utils.limiter")
    limiter_module.RateLimiter = lambda qps: MagicMock()
    install("kiritop115strmhelper.utils.limiter", limiter_module)

    module_path = (
        Path(__file__).resolve().parents[1]
        / "helper/strm/share/audit_download_queue.py"
    )
    module_name = "kiritop115strmhelper.helper.strm.share.audit_download_queue"
