...
def _load_r302_module() -> Any:
    ...
    open_module = ModuleType("kiritop115strmhelper.core.u115_open")
    open_module.U115OpenHelper = MagicMock
    install("kiritop115strmhelper.core.u115_open", open_module)
    ...
    http_module = ModuleType("kiritop115strmhelper.utils.http")
    http_module.check_response = lambda response: response
    install("kiritop115strmhelper.utils.http", http_module)

    url_module = ModuleType("kiritop115strmhelper.utils.url")
    url_module.Url = _Url
    install("kiritop115strmhelper.utils.url", url_module)

    sentry_module = ModuleType("kiritop115strmhelper.utils.sentry")
    sentry_module.sentry_manager = SimpleNamespace(
        capture_all_class_exceptions=lambda cls: cls
    )
    install("kiritop115strmhelper.utils.sentry", sentry_module)

    module_path = (
        Path(__file__).resolve().parents[1] / "helper" / "r302" / "__init__.py"
    )
    module_name = "kiritop115strmhelper.helper.r302"
