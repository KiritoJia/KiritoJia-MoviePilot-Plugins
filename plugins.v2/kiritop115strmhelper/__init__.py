...
class KiritoP115StrmHelper(_PluginBase):
    """
    115网盘STRM助手插件入口，提供STRM生成、分享转存、离线下载、302跳转、FUSE挂载等功能
    """

    # 插件名称
    plugin_name = "Kirito 115网盘STRM助手"
    # 插件描述
    ...
    # 插件配置项ID前缀
    plugin_config_prefix = "kiritop115strmhelper_"
    # 加载顺序
    plugin_order = 99
    # 可使用的用户级别
    auth_level = 1
    ...
    def logs_oper(oper_name: str):
        ...
        def decorator(func):
            ...
            def wrapper(self, *args, **kwargs):
                ...
                :param self: KiritoP115StrmHelper 实例
                :param args: 位置参数
                :param kwargs: 关键字参数
    ...
    def get_service(self) -> List[Dict[str, str | Dict[Any, Any] | Any]] | None:
        ...
        cron_service = [
            {
                "id": "KiritoP115StrmHelper_offline_status",
                "name": "监控115网盘离线下载进度",
        ...
        if (
            ...
            cron_service.append(
                {
                    "id": "KiritoP115StrmHelper_monitor_life_guard",
                    "name": "115生活事件线程守护",
        ...
        if (
            ...
            cron_service.append(
                {
                    "id": "KiritoP115StrmHelper_full_sync_strm_files",
                    "name": "定期全量同步115媒体库",
        ...
        if (
            ...
            cron_service.append(
                {
                    "id": "KiritoP115StrmHelper_share_strm_cleanup",
                    "name": "定时分享STRM失效清理扫描",
                    "trigger": CronTrigger.from_crontab(
        ...
        if configer.cron_clear and (
            ...
            cron_service.append(
                {
                    "id": "KiritoP115StrmHelper_main_cleaner",
                    "name": "定期清理115空间",
        ...
        if (
            ...
            cron_service.append(
                {
                    "id": "KiritoP115StrmHelper_increment_sync_strm",
                    "name": "115网盘定期增量同步",
        ...
        if (
            ...
            cron_service.append(
                {
                    "id": "KiritoP115StrmHelper_hdhive_checkin",
                    "name": "HDHive 签到调度",
        ...
        if configer.enabled and configer.p115_checkin_enabled:
            cron_service.append(
                {
                    "id": "KiritoP115StrmHelper_p115_checkin",
                    "name": "115 签到调度",
                    "trigger": CronTrigger.from_crontab("*/5 * * * *"),
        ...
        if configer.strm_backup_enabled and configer.strm_backup_items:
            for backup_item in configer.strm_backup_items:
                if (
                    ...
                    cron_service.append(
                        {
                            "id": f"KiritoP115StrmHelper_strm_backup_{backup_item.name}",
...
"""
    ...
    def rename_dict_supplement(self, event: Event) -> None:
        ...
        def share_strm_center(url: str) -> Optional[Dict[str, Any]]:
            ...
            """
            for i in ["KiritoP115StrmHelper", "share_code=", "receive_code=", "id="]:
                if i not in url:
                    return None
            try:
        ...
        def resolve_media_info(media_path: str, media_item: FileItem) -> Dict[str, Any]:
            ...
            if media_item.storage == "local":
                ...
                params["url"] = (
                    ...
                    f"KiritoP115StrmHelper/redirect_url/{media_item.fileid}"
                )
    ...
    def intercept_if_exists_in_library(self, event: Event) -> None:
        ...
        data.cancel = True
        data.source = "KiritoP115StrmHelper"
    ...
    def share_strm_overwrite_check(self, event: Event) -> None:
        ...
        if (
            "KiritoP115StrmHelper" not in url
