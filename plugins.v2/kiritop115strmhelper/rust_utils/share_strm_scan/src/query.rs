...
const NEEDLE_P115: &[u8] = b"KiritoP115StrmHelper";
...
mod tests {
    ...
    fn gate_requires_plugin_name() {
        ...
        assert!(gate_share_strm(
            b"http://h/api/v1/plugin/KiritoP115StrmHelper/x?share_code=a&receive_code=b"
        ));
    }

    #[test]
    fn extract_query_fallback_like_python() {
        let line_with_plugin = "fooKiritoP115StrmHelper?share_code=x&receive_code=y";
    ...
    #[test]
    fn full_url_parse() {
        let line = "http://127.0.0.1:3000/api/v1/plugin/KiritoP115StrmHelper/redirect_url?share_code=sc&receive_code=rc&id=1";
