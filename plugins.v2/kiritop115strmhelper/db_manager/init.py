...
from ..db_manager import ct_db_manager, KiritoP115StrmHelperBase
...
def init_db(engine):
    ...
    # 全量建表
    KiritoP115StrmHelperBase.metadata.create_all(engine)
...
def migration_db(db_path, script_location, version_locations: list):
    ...
    """
    # 启动时的更新，调整从 /config/plugins/kiritop115strmhelper/database/versions 中读取持久化的迁移脚本)
    meta_data_path = Path(
        settings.ROOT_PATH
        / "app"
        / "plugins"
...
def init_migration_scripts() -> bool:
    ...
    target_path = Path(
        settings.PLUGIN_DATA_PATH / "kiritop115strmhelper" / "database" / "versions"
    )
