...
class ServiceHelper:
    ...
    def is_background_active(self) -> bool:
        ...
        return any(
            job.get("running") and job.get("pid") == "KiritoP115StrmHelper" for job in jobs
        )
    ...
    def _start_monitor_life_internal(self, register_guard_service: bool = True):
        ...
        if (
            ...
            self.monitor_life_thread = Thread(
                ...
                name="KiritoP115StrmHelper-MonitorLife",
                daemon=False,
            )
    ...
    def _update_monitor_life_guard_service(self):
        ...
        只重新注册115生活事件线程守护服务
        """
        pid = "KiritoP115StrmHelper"
        service_id = "KiritoP115StrmHelper_monitor_life_guard"
            ...
            for item in configer.directory_upload_path or []:
                ...
                try:
                    ...
                    watch_thread = Thread(
                        ...
                        name=f"KiritoP115StrmHelper-DirectoryUpload-{mon_path}",
                        daemon=True,
                    )
                    watch_thread.start()
